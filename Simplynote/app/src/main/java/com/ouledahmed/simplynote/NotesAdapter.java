...
public class NotesAdapter extends RecyclerView.Adapter<NotesAdapter.MyViewHolder> {
    ...
    public NotesAdapter(Context context, ArrayList note_id, ArrayList note_title, ArrayList note_description,
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, final int position) {
        ...
        holder.cardLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                NotesFragmentDirections.ActionNotesFragmentToReadFragment actionNotesFragmentToReadFragment =
                        NotesFragmentDirections.actionNotesFragmentToReadFragment();




                String id = String.valueOf(note_id.get(position));
                actionNotesFragmentToReadFragment.setIdArg(Integer.valueOf(id));

                actionNotesFragmentToReadFragment.setTitleArg(String.valueOf(note_title.get(position)));
                actionNotesFragmentToReadFragment.setDescriptionArg(String.valueOf(note_description.get(position)));
                actionNotesFragmentToReadFragment.setContentArg(String.valueOf(note_content.get(position)));

                if (String.valueOf(note_is_important.get(position)).equals("0")){
                    actionNotesFragmentToReadFragment.setIsImportantArg(0);
                } else if (String.valueOf(note_is_important.get(position)).equals("1")){
                    actionNotesFragmentToReadFragment.setIsImportantArg(1);
                }

                if (String.valueOf(note_is_urgent.get(position)).equals("0")){
                    actionNotesFragmentToReadFragment.setIsUrgentArg(0);
                } else if (String.valueOf(note_is_urgent.get(position)).equals("1")){
                    actionNotesFragmentToReadFragment.setIsUrgentArg(1);
                }

                Navigation.findNavController(view).navigate(actionNotesFragmentToReadFragment);
            }
        });
